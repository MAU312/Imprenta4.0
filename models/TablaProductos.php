...
public function contarRegistrosFiltrados($searchValue)
{
    ...
    {
        $sql = "SELECT SUM(Valor_Inventario) AS total FROM materiales";
    ...
    // Método para obtener el total de eventos ocurridos en el último mes
    public function obtenerImportacionesMes()
    {
        // Consulta SQL para contar los eventos ocurridos en el último mes
        $query = "
            SELECT COUNT(*) AS total
            FROM events
            WHERE start >= DATE_SUB(CURRENT_DATE, INTERVAL 1 MONTH)
        ";
        ...
        // Devolver el total de eventos en el último mes
        if ($row) {
            return $row['total'];
        }
